...
pub async fn seed_user(id: &str, email: &str, valence: &Valence) {
    ...
    .expect("build user");
    User::upsert_used(id, user, valence, valence::use_!("upsert User in tag/tests/helpers.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await.expect("upsert user");
}
